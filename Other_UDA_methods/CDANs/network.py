...
class RandomLayer(nn.Module):   # 将输出的预测概率映射到与特征的相同空间中
...
#     layer = RandomLayer(input_dim_list=[1024, 7])
#     feature = torch.randn((4, 1024))
#     softmax_out = torch.randn((4, 7))
#     rout = layer([feature, softmax_out])
#     a = 11
...
class ResNetFc(nn.Module):
  def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
    ...
    # pretrained_dict = torch.load(r'F:\project\DIFC\pre_trained/resnet50_pretrained_on_msceleb.pth.tar')
    # model_dict = self.feature_layers.state_dict()
    #
    ...
    # self.feature_layers.load_state_dict(model_dict)
...
if __name__ == '__main__':
    ...
    model = ResNetFc('ResNet50', use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=7)
    '''计算FPS'''
    # t_total = []
    source = torch.randn((4, 3, 224, 224))
    # for i in range(100):
    #     # c_trg = c_trg_list[0]
    #     # c_org = c_org_list[0]
    #     torch.cuda.synchronize()
    #     time_start = time.time()
    #     out, feat = model(source)
    #     torch.cuda.synchronize()
    #     time_end = time.time()
    #     time_sum = time_end - time_start
    #     # print(time_sum)
    #     t_total.append(time_sum)
    # print('average time:', np.mean(t_total) / 1)
    # print('average fps:', 1 / np.mean(t_total))
    # break
    '''参数和浮点数计算'''
    # 计算参数量和FLOPs
    flops, params = profile(model, inputs=(source,))
    ...
    # 转换单位：
    # 1 G = 10^9，1 M = 10^6，1 K = 10^3
    flops_g = flops / 1e9  # 转换为G FLOPs
    params_m = params / 1e6  # 转换为M参数

    # 打印结果，保留两位小数
    print(f"模型FLOPs: {flops_g:.3f} G")
    print(f"模型参数量: {params_m:.3f} M")
